...
opp_hash_table_t global_db; // TODO create partitioned db in future
...
static int async_db_op_helper(aroop_txt_t*key, aroop_txt_t*newval, aroop_txt_t*expval) {
	if(aroop_txt_is_empty(key) || aroop_txt_is_empty(newval)) { // we cannot process the request ..
		return -1;
	}
...
static int async_db_hook_desc(aroop_txt_t*plugin_space, aroop_txt_t*output) {
	return plugin_desc(output, "async db", "db", plugin_space, __FILE__, "It helps cas and other rpc db values in master process.\n");
}
